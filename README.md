...
Live : https://pierre-claisse.github.io/duan-yuting/
...
`npm ci` → `node scripts/build-secrets.mjs` → `VITE_BASE=/duan-yuting/ npm run build`
→ déploiement Pages.

Secrets CI requis sur le dépôt `duan-yuting` : `SYNC_PAT`, `SYNC_PASSWORD`.
