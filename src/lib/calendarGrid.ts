...
function weekdayMondayFirst(year: number, month: number): number {
  const sundayFirst = new Date(Date.UTC(year, month - 1, 1)).getUTCDay(); // 0=Sunday
  return (sundayFirst + 6) % 7;
}

/**
 * Build a 6x7 calendar grid (42 cells) for the given month, starting Monday.
...
export function buildCalendarGrid(year: number, month: number): CalendarCell[] {
  ...
  for (let day = 1; day <= inMonthDays; day++) {
    cells.push({ date: `${year}-${pad2(month)}-${pad2(day)}`, day, inMonth: true });
  }
